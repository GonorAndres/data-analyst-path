...
export function KPITooltip({ content, hintUp, hintDown, direction = 'up', children }: KPITooltipProps) {
  ...
  return (
    <div className="relative" tabIndex={0} aria-describedby={visible ? tooltipId : undefined} onFocus={show} onBlur={hide} onKeyDown={event => { if (event.key === 'Escape') hide() }} onMouseEnter={show} onMouseLeave={hide}>
      {children}
      <AnimatePresence>
        {visible && (
          <motion.div
            ...
            className="absolute bottom-[calc(100%+8px)] left-0 z-50 w-64 max-w-[calc(100vw-3rem)] glass-card p-4 shadow-xl pointer-events-none"
          >
