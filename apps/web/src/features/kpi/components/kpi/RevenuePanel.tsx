...
import { useRevenue } from '@/features/kpi/hooks/useKPIAPI'
...
import type { RevenueResponse, WaterfallItem, MRRMovement } from '@/features/kpi/types/kpi-types'
...
function WaterfallChart({ data }: { data: WaterfallItem[] }) {
  const { t } = useLanguage()
  const labels: Record<string, string> = { 'Starting MRR': t('revenue.starting'), New: t('revenue.new'), Expansion: t('revenue.expansion'), Contraction: t('revenue.contraction'), Churned: t('revenue.churned'), 'Ending MRR': t('revenue.ending') }
  data = data.map(item => ({ ...item, name: labels[item.name] ?? item.name }))
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <LineChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
        ...
        <Line type="monotone" dataKey="target" stroke={CHART_COLORS.forecast} strokeWidth={1.5} strokeDasharray="5 5" dot={false} name={t('forecast.target')} />
      </LineChart>
    </ResponsiveContainer>
  )
}
  ...
  return (
    <ResponsiveContainer width="100%" height={250}>
      <LineChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
        ...
        />
        <ReferenceLine y={1} stroke="var(--chart-tick)" strokeDasharray="3 3" label={{ value: '100%', position: 'right', fontSize: 10, fill: 'var(--chart-tick)' }} />
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-[var(--glass-border)]">
            <th className="text-left py-2 px-3 text-[var(--fg-muted)] text-xs font-semibold">{text('Month', 'Mes')}</th>
...
export function RevenuePanel() {
  ...
  if (isLoading || error || !r) return <ChartState loading={isLoading} error={error} empty={!r} />
