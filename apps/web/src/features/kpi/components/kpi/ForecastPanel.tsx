...
import { useForecast } from '@/features/kpi/hooks/useKPIAPI'
...
import type { ForecastResponse, ForecastMetric } from '@/features/kpi/types/kpi-types'
          ...
          {forecasts.map((f) => {
            ...
            return (
              <tr key={f.metric} className="border-b border-[var(--glass-border)] hover:bg-[var(--glass-bg)]">
                <td className="py-2 px-3 text-[var(--fg-primary)] font-medium">{metricLabel(f.metric, t, f.metric_label)}</td>
...
export function ForecastPanel() {
  ...
  if (isLoading || error || !f) return <ChartState loading={isLoading} error={error} empty={!f} />
  ...
  return (
    <div className="space-y-6">
      ...
      {f.forecasts.map((forecast) => (
        <ChartContainer
          key={forecast.metric}
          title={`${metricLabel(forecast.metric, t, forecast.metric_label)} ${t('forecast.predicted')}`}
