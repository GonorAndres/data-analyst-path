...
import { KPICard } from '@/features/kpi/components/ui/KPICard'
import type { OverviewResponse, KPIMetric } from '@/features/kpi/types/kpi-types'
import type { TranslationKey } from '@/features/kpi/lib/translations'
import { metricKey, metricLabel } from '@/features/kpi/lib/metric-labels'
...
export function OverviewPanel() {
  const { queryString, filters } = useKPIFilters()
  const { t } = useLanguage()
  ...
  return (
    <div className="space-y-8">
      ...
      {!isLoading && !error && !o && <ChartState empty />}
      {!isLoading && !error && o && (
        <>
          {/* Health Score + Period */}
          ...
          {categories.map((cat) => {
            const kpis = o.kpis.filter((k: KPIMetric) => (k.category === 'customers' ? 'customer' : k.category === 'engagement' ? 'performance' : k.category) === cat.key)
              ...
              <div key={cat.key}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {kpis.map((kpi: KPIMetric, i: number) => (
                    <KPICard
                      key={kpi.id}
                      metric={{ ...kpi, name: metricLabel(kpi.id, t, kpi.name) }}
                      delay={i * 0.08}
                      comparison={filters.comparison === 'yoy' ? 'yoy' : 'mom'}
                      tooltip={t(`tooltip.${metricKey(kpi.id)}` as TranslationKey)}
                      direction={KPI_DIRECTION[metricKey(kpi.id)] ?? 'neutral'}
