...
export function ChoroplethMap({ data, metric, onBoroughClick }: ChoroplethMapProps) {
  ...
  useEffect(() => {
    ...
    const colorScale = d3.scaleSequential(
      (t: number) => d3.interpolateRgb(chartColor('--ops-surface'), chartColor('--ops-blue'))(t)
    ...
    boroughs.forEach(([name, { path, labelX, labelY }]) => {
      ...
      group
        .append('path')
        .attr('d', path)
        .attr('fill', val > 0 ? colorScale(val) : 'var(--ops-surface-hover)')
        .attr('stroke', 'var(--ops-border)')
        .attr('stroke-width', 1.5)
        .attr('cursor', onBoroughClick ? 'pointer' : 'default')
        .attr('tabindex', onBoroughClick ? 0 : -1)
        .attr('role', onBoroughClick ? 'button' : 'img')
        .attr('aria-label', `${name}: ${val}`)
        .on('keydown', function (event: KeyboardEvent) {
          if (onBoroughClick && (event.key === 'Enter' || event.key === ' ')) {
            event.preventDefault()
            onBoroughClick(name)
          }
        })
        .on('mouseenter', function (event: MouseEvent) {
          d3.select(this).attr('stroke', 'var(--ops-text)').attr('stroke-width', 2)
          tooltip
            .style('display', 'block')
            .style('left', `${event.offsetX + 12}px`)
            .style('top', `${event.offsetY - 28}px`)
            .html(
              `<span style="color:var(--ops-text);font-weight:600">${name}</span><br/><span style="color:var(--ops-text-muted)">${metric}: ${d3.format(',')(val)}</span>`
            )
        })
        ...
        .on('mouseleave', function () {
          d3.select(this).attr('stroke', 'var(--ops-border)').attr('stroke-width', 1.5)
  ...
  }, [theme, locale, paletteRevision, chartWidth, data, metric, onBoroughClick])
