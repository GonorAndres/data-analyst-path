...
export function SeasonalityChart({ data, width: propWidth, height: propHeight }: SeasonalityChartProps) {
  ...
  useEffect(() => {
    ...
    const color = d3
      .scaleSequential()
      .domain([0, maxVal])
      .interpolator(d3.interpolateRgb(chartColor('--ops-bg'), chartColor('--ops-blue')))

    // Draw cells
    data.days.forEach((day, i) => {
      data.hours.forEach((hour, j) => {
        const val = data.matrix[i]?.[j] ?? 0
        const dayLabel = translateFeature(DAY_LABELS[i] ?? day, locale)
        ...
        g.append('rect')
          ...
          .on('mouseenter', function (event: MouseEvent) {
            d3.select(this).attr('stroke', 'var(--ops-text)').attr('stroke-width', 1)
            tooltip
              .style('display', 'block')
              .style('left', `${event.offsetX + 12}px`)
              .style('top', `${event.offsetY - 28}px`)
              .html(
                `<span style="color:var(--ops-text);font-weight:600">${dayLabel} ${String(hour).padStart(2, '0')}:00</span><br/><span style="color:var(--ops-blue)">${d3.format(',')(val)} ${translateFeature('solicitudes', locale)}</span>`
              )
          })
    ...
    g.selectAll('.day-label')
      ...
      .attr('font-size', '10px')
      .text((_, i) => translateFeature(DAY_LABELS[i] ?? data.days[i], locale))
