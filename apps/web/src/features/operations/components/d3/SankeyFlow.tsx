...
const COLUMN_COLORS = ['var(--ops-blue)', 'var(--ops-cyan)', 'var(--ops-green)', 'var(--ops-amber)']
...
export function SankeyFlow({ data, width: propWidth, height: propHeight }: SankeyFlowProps) {
  ...
  useEffect(() => {
    ...
    linkGroup
      ...
      .attr('stroke', (_d: any) => 'var(--ops-text-muted)')
      ...
      .on('mouseenter', function (event: MouseEvent, d: any) {
        ...
        tooltip
          ...
          .html(
            `<span style="color:var(--ops-text-muted)">${srcName}</span> → <span style="color:var(--ops-text-muted)">${tgtName}</span><br/><span style="color:var(--ops-text);font-weight:600">${d3.format(',')(d.value)}</span>`
          )
      })
    ...
    nodeGroup
      ...
      .attr('rx', 0)
      .attr('stroke', (d: any) => d.value === maxValue ? 'var(--ops-text)' : 'none')
      .attr('stroke-width', 1)
      .on('mouseenter', function (event: MouseEvent, d: any) {
        tooltip
          .style('display', 'block')
          ...
          .style('top', `${event.offsetY - 28}px`)
          .html(
            `<span style="color:var(--ops-text);font-weight:600">${d.name}</span><br/><span style="color:var(--ops-text-muted)">${translateFeature('Volumen:', locale)} ${d3.format(',')(d.value)}</span>`
          )
      })
      .on('mousemove', function (event: MouseEvent) {
        tooltip
