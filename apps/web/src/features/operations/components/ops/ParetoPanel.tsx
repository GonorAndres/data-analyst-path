...
import { useOpsPareto } from '@/features/operations/hooks/useOpsAPI'
...
import { ParetoChart } from '@/features/operations/components/d3/ParetoChart'
...
function PriorityScatter({ data }: { data: PriorityItem[] }) {
  ...
  useEffect(() => {
    ...
    function slaColor(sla: number) {
      if (sla >= 85) return 'var(--ops-green)'
      if (sla >= 70) return 'var(--ops-amber)'
      return 'var(--ops-red)'
    }
    ...
    svg
      ...
      .attr('font-size', '10px')
      .text(translateFeature('Volumen de solicitudes', locale))
    ...
    svg
      ...
      .attr('font-family', 'Inter')
      .attr('font-size', '10px')
      .text(translateFeature('Dias de resolucion prom.', locale))
    ...
    g.selectAll('circle')
      ...
      .on('mouseenter', function (event: MouseEvent, d) {
        ...
        tooltip
          ...
          .html(
            `<span style="color:var(--ops-text);font-weight:600">${d.complaint_type}</span><br/><span style="color:var(--ops-text-muted)">${translateFeature('Solicitudes', locale)}: ${d3.format(',')(d.volume)}</span><br/><span style="color:var(--ops-text-muted)">${translateFeature('Resolucion Prom.', locale)}: ${d.avg_resolution_days == null ? '—' : d.avg_resolution_days.toFixed(1)}d</span><br/><span style="color:${slaColor(d.sla_compliance ?? 0)}">SLA: ${d.sla_compliance != null ? d.sla_compliance.toFixed(1) + '%' : 'N/A'}</span>`
          )
      })
    ...
    // Legend
    const legendData = [
      { label: 'SLA >= 85%', color: 'var(--ops-green)' },
      { label: 'SLA >= 70%', color: 'var(--ops-amber)' },
      { label: 'SLA < 70%', color: 'var(--ops-red)' },
    ]
...
export function ParetoPanel() {
  ...
  const thresholdIndex = paretoItems.findIndex(
    (item: { cumulative_pct: number }) => item.cumulative_pct >= 80
  )
  const totalTypes = paretoItems.length
  const pctTypes =
    totalTypes > 0 && thresholdIndex >= 0 ? (((thresholdIndex + 1) / totalTypes) * 100).toFixed(0) : '—'
          ...
          <>
            ...
            <div className="mt-3 px-1">
              <p className="font-sans text-xs text-ops-text-muted">
                ...
                {pctTypes}% de tipos de queja genera al menos el 80% del volumen total.
              </p>
            </div>
          </>
        ) : (
