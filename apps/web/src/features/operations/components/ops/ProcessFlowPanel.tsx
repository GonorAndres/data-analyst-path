...
import { useOpsBotleneck } from '@/features/operations/hooks/useOpsAPI'
...
import { SankeyFlow } from '@/features/operations/components/d3/SankeyFlow'
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {bottlenecks.map((row, i) => {
                  ...
                  return (
                    <FeatureText key={`${row.agency}-${row.complaint_type}-${i}`}><tr
                    ...
                    >
                      ...
                      <td className="font-mono text-ops-text text-right py-2 pr-4 tabular-nums">
                        {row.avg_days == null ? '—' : row.avg_days.toFixed(1)}
                      </td>
                      <td className="font-mono text-ops-text text-right py-2 pr-4 tabular-nums">
                        {row.median_days == null ? '—' : row.median_days.toFixed(1)}
                      </td>
                      <td className="font-mono text-ops-text text-right py-2 pr-4 tabular-nums">
                        {row.count == null ? '—' : row.count.toLocaleString('es-MX')}
                      </td>
                      <td
