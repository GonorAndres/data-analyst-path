...
export function ProcesoTecnicoPanel() {
  ...
  return (
    <FeatureText><div className="space-y-4">
      ...
      {/* Notebook iframe */}
      <p className="text-xs text-ops-text-muted">{t('Source notebooks retain their original language and formatting.', 'Los notebooks fuente conservan su idioma y formato originales.')}</p>
      <div className="border border-ops-border">
        <iframe
          key={current.id}
          src={`/operations/notebooks_html/${current.file}`}
          className="w-full border-0"
          style={{ height: '800px', background: 'var(--ops-surface)' }}
          title={current.title}
        />
      </div>
