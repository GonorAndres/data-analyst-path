  ...
  return (
    <div className="border border-border rounded-lg mt-6">
      <button
        ...
        className="w-full flex items-center justify-between px-5 py-3 font-sans text-sm text-muted hover:text-ink transition-colors cursor-pointer"
      >
        <span className="tracking-wide">{tx("Acerca de los datos")}</span>
        <ChevronDown
          size={16}
      ...
      {open && (
        <div className="px-5 pb-5 border-t border-border">
          ...
          <div className="flex flex-wrap gap-x-8 gap-y-2 font-sans text-xs text-muted mb-4">
            ...
            <span>{tx("Periodo:") + " "}{period}</span>
            <span>{tx("Registros:") + " "}{records}</span>
          </div>

          <div>
            <p className="font-sans text-xs text-muted font-medium mb-1">{tx("Limitaciones:")}</p>
