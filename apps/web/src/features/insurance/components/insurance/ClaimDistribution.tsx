            ...
            content={({ active, payload }) => {
              ...
              return (
                <div style={{
                  ...
                  <p>{tx("Siniestros:") + " "}{row.count.toLocaleString()}</p>
                </div>
              )
            }}
          />
      ...
      {data.report_lag_stats && data.report_lag_stats.length > 0 && (
        <div className="mt-8">
          <h3 className="font-serif text-lg text-ink mb-3">
            {tx("Rezago de reporte por línea de negocio")}</h3>
          <div className="overflow-x-auto">
            <div className="max-w-full overflow-x-auto"><table className="w-full font-sans text-xs">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left py-2 pr-4 text-muted font-medium tracking-wide uppercase">{tx("Línea")}</th>
                  <th className="text-right py-2 px-4 text-muted font-medium tracking-wide uppercase">{tx("Promedio (días)")}</th>
                  <th className="text-right py-2 pl-4 text-muted font-medium tracking-wide uppercase">{tx("Mediana (días)")}</th>
                </tr>
              </thead>
              <tbody>
                {data.report_lag_stats.map((row, i) => (
                  <tr key={i} className="border-b border-border">
                    <td className="py-2 pr-4 text-ink">{row.line_of_business}</td>
                    <td className="text-right py-2 px-4 text-ink tabular-nums">{row.mean_lag.toFixed(0)}</td>
                    <td className="text-right py-2 pl-4 text-ink tabular-nums">{row.median_lag.toFixed(0)}</td>
                  </tr>
                ))}
              </tbody>
            </table></div>
