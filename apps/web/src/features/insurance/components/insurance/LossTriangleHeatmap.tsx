  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <div className="flex flex-wrap gap-4 mb-4">
        <div className="flex gap-2">
          {(['incurred', 'paid'] as ViewMode[]).map(mode => (
            ...
            >
              {mode === 'incurred' ? tx("Incurrido") : tx("Pagado")}
            </button>
          ))}
        </div>
        <div className="flex gap-2">
      ...
      <p className="font-sans text-xs text-muted mt-1">
        {tx("Filas = año de accidente. Columnas = lag de desarrollo. Vista:") + " "}{viewMode === 'incurred' ? tx("pérdidas incurridas acumuladas") : tx("pérdidas pagadas acumuladas")}.
      </p>

      {/* Tooltip */}
      {tooltip.visible && (
        <div
          className="absolute pointer-events-none bg-paper border border-border px-3 py-2 z-10"
          style={{ left: tooltip.x + 12, top: tooltip.y - 40 }}
        >
          <p className="font-sans text-xs text-muted">{tx("Año") + " "}{tooltip.accidentYear} -- Lag {tooltip.lag}</p>
          <p className="font-sans text-sm font-medium text-ink">${tooltip.value.toLocaleString()}</p>
        </div>
      )}
    </div>
  )
