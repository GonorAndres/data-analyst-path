          ...
          content={({ active, payload, label }) => {
            ...
            return (
              <div style={{
                ...
                <p style={{ fontWeight: 600, marginBottom: 6 }}>{tx("Año:") + " "}{label}</p>
                {freq != null && <p>{tx("Frecuencia:") + " "}{freq.toFixed(4)}</p>}
                {sev != null && <p>{tx("Severidad:") + " "}{formatAmount(sev)}</p>}
              </div>
            )
          }}
        />
        <Legend
          formatter={(value: string) => {
            const labels: Record<string, string> = {
              frequency: tx("Frecuencia"),
              severity: tx("Severidad promedio"),
            }
