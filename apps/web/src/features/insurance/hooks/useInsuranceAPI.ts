...
import { insuranceFetcher } from '@/features/insurance/lib/insurance-api'
...
export function useInsuranceKPIs(filters: InsuranceFilters) {
  return useSWR(`/api/insurance/api/v1/kpis${buildQuery(filters)}`, insuranceFetcher, SWR_CONFIG)
}

export function useLossTriangle(filters: InsuranceFilters, type: 'incurred' | 'paid' = 'incurred', method: 'cl' | 'bf' = 'cl') {
  return useSWR(`/api/insurance/api/v1/loss-triangle${buildQuery(filters)}&type=${type}&method=${method}`, insuranceFetcher, SWR_CONFIG)
}
...
export function useFrequencySeverity(filters: InsuranceFilters) {
  return useSWR(`/api/insurance/api/v1/frequency-severity${buildQuery(filters)}`, insuranceFetcher, SWR_CONFIG)
}

export function useLossRatios(filters: InsuranceFilters) {
  return useSWR(`/api/insurance/api/v1/loss-ratios${buildQuery(filters)}`, insuranceFetcher, SWR_CONFIG)
}

export function useCombinedRatio(filters: InsuranceFilters) {
  return useSWR(`/api/insurance/api/v1/combined-ratio${buildQuery(filters)}`, insuranceFetcher, SWR_CONFIG)
}

export function useClaimDistribution(filters: InsuranceFilters) {
  return useSWR(`/api/insurance/api/v1/claim-distribution${buildQuery(filters)}`, insuranceFetcher, SWR_CONFIG)
}

export function useInsuranceFilters() {
  return useSWR('/api/insurance/api/v1/filters', insuranceFetcher, { ...SWR_CONFIG, revalidateOnMount: true })
}
