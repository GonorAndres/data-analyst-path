...
import { formatCohort, formatPercent, formatPercentTick } from '@/features/cohorts/lib/format'
import type { CurvePoint, RetentionRow } from '@/features/cohorts/lib/filters'
    ...
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={data} margin={{ top: 12, right: 16, left: 4, bottom: 4 }}>
        ...
        <Legend
          ...
          payload={[
            { value: tx("Promedio de cohortes"), type: 'line', color: 'var(--series-1)' },
            { value: tx("IC 95%"), type: 'rect', color: 'var(--series-1)' },
            { value: tx("Cohortes individuales"), type: 'line', color: 'var(--chart-tick)' },
          ]}
        />
        ...
        {rows.map((row) => (
          <Line
            ...
            name={tx(formatCohort(row.cohort))}
            stroke="var(--series-1)"
            strokeOpacity={0.1}
            strokeWidth={1}
            dot={false}
      ...
      <ComposedChart data={data} margin={{ top: 12, right: 16, left: 4, bottom: 4 }}>
        ...
        {showExtremes && (
          <Line
            type="linear"
            dataKey="best"
            name={(tx("Mejor ·") + " " + (tx(formatCohort(best))) + "")}
            stroke="var(--series-3)"
            strokeWidth={2}
        ...
        {showExtremes && (
          <Line
            type="linear"
            dataKey="worst"
            name={(tx("Peor ·") + " " + (tx(formatCohort(worst))) + "")}
            stroke="var(--series-5)"
            strokeWidth={2}
