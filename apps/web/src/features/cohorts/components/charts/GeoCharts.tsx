...
import { formatExact, formatPercent, formatPercentTick } from '@/features/cohorts/lib/format'
import type { StateRow } from '@/features/cohorts/lib/filters'
        ...
        <Tooltip
          ...
          content={
            <ChartTooltip
              ...
              labelFormatter={(l) => (tx("Estado") + " " + (l) + "")}
            />
          }
        />
        <ReferenceLine x={median} stroke="var(--chart-label)" strokeDasharray="3 3">
          <Label
            value={(tx("Mediana nacional") + " " + (median.toFixed(1)) + "%")}
            position="insideTopRight"
            fill="var(--chart-label)"
            fontSize={11}
          />
        </ReferenceLine>
        <Bar dataKey="repeatPct" name={tx("Tasa de recompra")} radius={[0, 4, 4, 0]} maxBarSize={18}>
          {sorted.map((d) => (
            <Cell
              key={d.state}
              fill="var(--series-1)"
    ...
    <ResponsiveContainer width="100%" height="100%">
      <LineChart data={data} margin={{ top: 12, right: 16, left: 4, bottom: 8 }}>
        ...
        <XAxis
          ...
          {...axis}
          label={{
            value: tx("Meses desde la primera compra"),
    ...
    <ResponsiveContainer width="100%" height="100%">
      <ScatterChart margin={{ top: 16, right: 24, left: 4, bottom: 12 }}>
        ...
        <XAxis
          ...
          {...axis}
          label={{
            value: tx("Días promedio de entrega"),
            position: 'insideBottom',
            offset: -8,
            fill: 'var(--chart-tick)',
            fontSize: 11,
        ...
        <YAxis
          ...
          {...axis}
        />
        <ZAxis type="number" dataKey="customers" range={[40, 520]} name={tx("Clientes")} />
        <Tooltip
          ...
          content={({ active, payload }) => {
            ...
            return (
              ...
              >
                <p className="font-sans text-xs text-muted mb-1">{tx("Estado") + " "}{p.state}</p>
                <p className="font-sans text-sm text-[var(--chart-label)] tabular-nums">
                  {p.x.toFixed(1)} {" " + tx("días ·") + " "}{p.y.toFixed(2)}{tx("% recompra")}</p>
                <p className="font-sans text-xs text-muted tabular-nums">
                  {formatExact(p.customers)} {" " + tx("clientes")}</p>
              </div>
            )
          }}
        />
