...
import type { Segments } from '@/features/cohorts/lib/types'
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={sorted} layout="vertical" margin={{ top: 8, right: 48, left: 8, bottom: 4 }}>
        ...
        <YAxis type="category" dataKey="segment" tickFormatter={(value: string) => tx(value)} width={112} {...axis} />
        <Tooltip
          cursor={{ fill: 'var(--chart-grid)', fillOpacity: 0.35 }}
          content={<ChartTooltip valueFormatter={(v) => ("" + (formatExact(v)) + " " + tx("clientes"))} />}
        />
        <Bar dataKey="customers" name={tx("Clientes")} radius={[0, 4, 4, 0]} maxBarSize={22}>
          {sorted.map((d) => (
            <Cell
              key={d.segment}
              fill="var(--series-1)"
                ...
                return (
                  ...
                  >
                    ...
                    <p className="font-sans text-sm text-[var(--chart-label)] tabular-nums">
                      {formatExact(p.count)} {" " + tx("clientes")}</p>
                    <p className="font-sans text-xs text-muted tabular-nums">
                      {p.recency}–{p.recency + 29} {" " + tx("días ·") + " "}{p.frequency}
                      {p.frequency >= 5 ? '+' : ''} {" " + tx("pedido")}{p.frequency > 1 ? 's' : ''} ·{' '}
                      {formatCurrency(p.avg_revenue)} {" " + tx("medio")}</p>
                  </div>
                )
              }}
            />
    ...
    <ResponsiveContainer width="100%" height="100%">
      <LineChart data={points} margin={{ top: 12, right: 20, left: 4, bottom: 12 }}>
        ...
        <XAxis
          ...
          {...axis}
          label={{
            value: tx("Clientes acumulados (de menor a mayor gasto)"),
        ...
        <Tooltip
          ...
          content={
            <ChartTooltip
              ...
              labelFormatter={(l) => ("" + (Number(l).toFixed(0)) + tx("% de los clientes"))}
              only={['revenue']}
            />
          }
        />
        <Legend
          ...
          payload={[
            { value: tx("Ingresos acumulados"), type: 'line', color: 'var(--series-1)' },
            { value: tx("Igualdad perfecta"), type: 'line', color: 'var(--chart-tick)' },
          ]}
        />
        <ReferenceLine
          x={80}
          ...
          strokeDasharray="3 3"
          label={{
            value: (tx("20% superior =") + " " + (data.top20_share.toFixed(0)) + tx("% de los ingresos")),
