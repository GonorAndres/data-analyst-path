...
import { formatCohort, formatCurrency, formatExact } from '@/features/cohorts/lib/format'
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <AreaChart data={data} margin={{ top: 12, right: 16, left: 4, bottom: 4 }}>
        ...
        <XAxis dataKey="month" tickFormatter={(value: string) => tx(formatCohort(value))} interval="preserveStartEnd" {...axis} />
        <YAxis tickFormatter={(v) => formatCurrency(Number(v))} width={64} {...axis} />
        <Tooltip
          cursor={{ stroke: 'var(--chart-tick)', strokeWidth: 1 }}
          content={
            <ChartTooltip valueFormatter={(v) => formatCurrency(v)} labelFormatter={(value: string) => tx(formatCohort(value))} />
          }
        />
        ...
        {peak && (
          <ReferenceLine
            ...
            strokeDasharray="3 3"
            label={{
              value: (tx("Pico ·") + " " + (formatCurrency(peak.revenue)) + ""),
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={data} margin={{ top: 12, right: 16, left: 4, bottom: 4 }}>
        <CartesianGrid stroke="var(--chart-grid)" vertical={false} />
        <XAxis dataKey="cohort" tickFormatter={(value: string) => tx(formatCohort(value))} interval="preserveStartEnd" {...axis} />
        <YAxis tickFormatter={(v) => formatExact(Number(v))} width={52} {...axis} />
        <Tooltip
          cursor={{ fill: 'var(--chart-grid)', fillOpacity: 0.35 }}
          content={
            <ChartTooltip valueFormatter={(v) => formatExact(v)} labelFormatter={(value: string) => tx(formatCohort(value))} />
          }
        />
        ...
        <Bar dataKey="customers" name={tx("Clientes nuevos")} radius={[4, 4, 0, 0]} maxBarSize={26}>
          {data.map((d) => (
            <Cell
              key={d.cohort}
              fill="var(--series-1)"
