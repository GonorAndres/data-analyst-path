...
import type { Filters } from '@/features/cohorts/lib/types'
import { formatCohort } from '@/features/cohorts/lib/format'
...
export function FilterBar({ filters, cohorts, segments, defaults, onChange }: Props) {
  ...
  return (
    <div className="border border-border bg-[var(--chart-bg)] rounded-sm px-4 py-3">
      <div className="flex flex-wrap items-end gap-x-6 gap-y-3">
        <label className="flex flex-col gap-1">
          <span className="font-sans text-xs uppercase tracking-wider text-muted">{tx("Cohorte desde")}</span>
          <select
            value={filters.cohortStart}
            onChange={(e) => onChange({ ...filters, cohortStart: e.target.value })}
            className="font-sans text-sm bg-transparent border border-border
        ...
        <label className="flex flex-col gap-1">
          <span className="font-sans text-xs uppercase tracking-wider text-muted">{tx("hasta")}</span>
          <select
        ...
        <label className="flex flex-col gap-1 min-w-[190px]">
          <span className="font-sans text-xs uppercase tracking-wider text-muted">
            {tx("Cohorte mínima:") + " "}<span className="tabular-nums">{filters.minCohortSize}</span> {" " + tx("clientes")}</span>
          <input
            type="range"
            min={10}
            max={500}
