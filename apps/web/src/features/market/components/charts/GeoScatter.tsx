...
function priceToColor(price: number): string {
  if (price < 800) return 'var(--series-3)'
  if (price < 2000) return 'var(--series-1)'
  return 'var(--series-4)'
}
...
const CustomTooltip = ({ active, payload }: { active?: boolean; payload?: { payload: GeoPoint }[] }) => {
  ...
  return (
    <div style={{
      ...
      <p>{p.neighbourhood}</p>
      <p>MXN {p.price?.toLocaleString()}{tx("/noche")}</p>
      <p style={{ color: 'var(--chart-tick)' }}>{tx(ROOM_TYPE_ES[p.room_type] ?? p.room_type)}</p>
    </div>
  )
}
  ...
  return (
    <div>
      <ResponsiveContainer width="100%" height={420}>
        <ScatterChart margin={{ top: 4, right: 16, bottom: 20, left: 0 }}>
          <XAxis
            ...
            label={{ value: tx("Longitud"), position: 'insideBottom', offset: -10, fontSize: 11, fill: 'var(--chart-tick)', fontFamily: 'var(--font-lora)' }}
          />
          <YAxis
            dataKey="lat"
            type="number"
            ...
            axisLine={false}
            tickFormatter={v => v.toFixed(2)}
            label={{ value: tx("Latitud"), angle: -90, position: 'insideLeft', offset: 10, fontSize: 11, fill: 'var(--chart-tick)', fontFamily: 'var(--font-lora)' }}
          />
          <ZAxis range={[20, 20]} />
      ...
      <div className="flex gap-6 mt-2 justify-center">
        {[
          { color: 'var(--series-3)', label: '< MXN 800' },
          { color: 'var(--series-1)', label: 'MXN 800–2,000' },
          { color: 'var(--series-4)', label: '> MXN 2,000' },
        ].map(({ color, label }) => (
