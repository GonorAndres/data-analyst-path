...
const ROOM_COLORS: Record<string, string> = {
  'Entire home/apt': 'var(--series-1)',
  'Private room': 'var(--series-4)',
  'Hotel room': 'var(--series-3)',
  'Shared room': 'var(--chart-tick)',
}
    ...
    <ResponsiveContainer width="100%" height={320}>
      <BarChart data={chartData} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
        ...
        <XAxis
          ...
          tickLine={false}
          axisLine={false}
          label={{ value: tx("Precio (MXN/noche)"), position: 'insideBottom', offset: -2, fontSize: 11, fill: 'var(--chart-tick)', fontFamily: 'var(--font-lora)' }}
        />
        <YAxis
          tick={{ fontSize: 11, fontFamily: 'var(--font-lora)', fill: 'var(--chart-tick)' }}
          tickLine={false}
        ...
        <Tooltip
          ...
          formatter={(value: number, name: string) => [value.toLocaleString(), tx(ROOM_TYPE_ES[name] ?? name)]}
          labelFormatter={label => `${label} MXN`}
        />
        {activeTypes.length > 1 && <Legend wrapperStyle={{ fontSize: 11, fontFamily: 'var(--font-lora)' }} formatter={(value: string) => tx(ROOM_TYPE_ES[value] ?? value)} />}
        {activeTypes.map(rt => (
          <Bar
            key={rt}
            dataKey={rt}
            fill={ROOM_COLORS[rt] ?? 'var(--series-1)'}
            opacity={0.85}
            radius={[1, 1, 0, 0]}
          />
        ))}
