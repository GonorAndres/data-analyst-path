  ...
  return (
    <div ref={containerRef} className="relative overflow-x-auto">
      ...
      <p className="font-sans text-xs text-muted mt-1">
        {tx("Mes 0 = cohorte de adquisición (100%). Los valores posteriores muestran qué % regresó a comprar.")}</p>
      ...
      {tooltip.visible && (
        ...
        >
          <p className="font-sans text-xs text-muted">{tooltip.cohort} {" " + tx("— Mes") + " "}{tooltip.month}</p>
          <p className="font-sans text-sm font-medium text-ink">{tooltip.retention.toFixed(1)}{tx("% retención")}</p>
        </div>
      )}
    </div>
  )
