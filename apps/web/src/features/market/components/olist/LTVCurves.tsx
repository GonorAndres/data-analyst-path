...
export function LTVCurves({ data, isLoading }: Props) {
  ...
  if (isLoading || !data) {
    return (
      <div className="animate-pulse bg-surface rounded" style={{ height: 300 }} />
    )
  }
  ...
  return (
    <div>
      <p className="font-sans text-xs text-muted mb-3">
        {tx("LTV acumulado al mes") + " "}{maxMonth}
      </p>
      ...
      <p className="font-sans text-xs text-muted mt-2">
        {tx("Método: LTV histórico (observado). Alternativas no incluidas: predictivo simple (AOV × frecuencia × vida útil) y probabilístico (BG/NBD + Gamma-Gamma para modelar abandono silencioso en negocios no contractuales).")}</p>
    </div>
  )
}
