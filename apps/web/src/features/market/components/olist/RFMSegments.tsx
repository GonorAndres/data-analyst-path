...
const CustomTooltip = ({ active, payload }: {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontWeight: 600, marginBottom: 6 }}>{tx(getSegmentLabel(s.segment))}</p>
      <p>{s.count.toLocaleString()} {" " + tx("clientes")}</p>
      <p>{tx("Recencia prom.:") + " "}{s.avg_recency.toFixed(0)} {" " + tx("dias")}</p>
      <p>{tx("Frecuencia prom.:") + " "}{s.avg_frequency.toFixed(1)}</p>
      <p>{tx("Valor prom.: R$") + " "}{s.avg_monetary.toLocaleString()}</p>
    </div>
  )
}
...
export function RFMSegments({ data, isLoading }: Props) {
  ...
  const displaySegments = data.segments.map(s => ({
    ...s,
    label: tx(getSegmentLabel(s.segment)),
  }))
  ...
  return (
    <div>
      ...
      <div>
        <p className="font-sans text-xs tracking-widest uppercase text-muted mb-4">{tx("Clientes por segmento")}</p>
        ...
        <div className="max-w-full overflow-x-auto"><table className="w-full text-left">
          <thead>
            <tr className="border-b border-border">
              {[tx("Segmento"), tx("Clientes"), tx("Recencia"), tx("Frecuencia"), tx("Valor")].map(h => (
          ...
          <tbody>
            {displaySegments.map(s => (
              <tr key={s.segment} className="border-b border-border">
      ...
      <p className="font-sans text-xs text-muted mt-4">
        {tx("Método: RFM clásico con puntuación por cuartiles (1–4) en cada dimensión. Puntuación combinada R+F+M (rango 3–12) asigna segmentos. En contextos con frecuencia promedio") + " "}{'>='}  {" " + tx("2, la variante RFM-T (con Tenure) o modelos probabilísticos como BG/NBD permiten estimar probabilidad de recompra — más apropiado para negocios contractuales o con ciclo de vida definido.")}</p>
    </div>
  )
}
