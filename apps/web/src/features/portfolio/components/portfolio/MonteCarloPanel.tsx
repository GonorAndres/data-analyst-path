...
import { useMonteCarlo } from '@/features/portfolio/hooks/usePortfolioAPI'
...
export function MonteCarloPanel() {
  ...
  const probProfit = (d?.prob_profit ?? Number.NaN) * 100
  const probTarget = (d?.prob_target ?? Number.NaN) * 100
    ...
    <FeatureText><div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6 glass-card p-6">
        <div>
          ...
          <input type="range" min={63} max={1260} step={63} aria-label="Horizon (Days)" value={days} onChange={(e) => setDays(Number(e.target.value))} className="w-full" />
          <p className="font-mono text-sm font-semibold tabular-nums text-accent mt-1">{days} {t('days', 'días')} ({(days / 252).toFixed(1)} {t('years', 'años')})</p>
        </div>
        <div>
          <label className="block font-sans text-[11px] tracking-widest uppercase text-muted mb-2">Simulations</label>
          <input type="range" min={100} max={5000} step={100} aria-label="Simulations" value={simulations} onChange={(e) => setSimulations(Number(e.target.value))} className="w-full" />
        ...
        <div>
          ...
          <input type="range" min={10000} max={1000000} step={10000} aria-label="Initial Value" value={initialValue} onChange={(e) => setInitialValue(Number(e.target.value))} className="w-full" />
        ...
        <div>
          ...
          <input type="range" min={0} max={100} step={5} aria-label="Target Return (%)" value={target} onChange={(e) => setTarget(Number(e.target.value))} className="w-full" />
      ...
      {d && !isLoading && !error && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <KPICard label="Median Final Value" value={stats.median ?? Number.NaN} prefix="$" decimals={0} trend="neutral" delay={0.16} tooltip="The middle outcome across all simulations, 50% chance of doing better or worse." />
            <KPICard label="5th Pctl (Worst Case)" value={stats.percentile_5 ?? Number.NaN} prefix="$" decimals={0} trend="down" delay={0.24} tooltip="Only 5% of simulations ended below this value." />
          </div>
