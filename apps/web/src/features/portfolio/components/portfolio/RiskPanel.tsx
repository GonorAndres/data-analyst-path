...
import { useRisk } from '@/features/portfolio/hooks/usePortfolioAPI'
...
const VAR_COLORS = ['var(--us-equity)', 'var(--benchmark)', 'var(--intl-equity)']
...
export function RiskPanel() {
  ...
  const stat = (value: number | undefined, digits: number, scale = 1) => typeof value === 'number' && Number.isFinite(value) ? (value * scale).toFixed(digits) : '—'
  ...
  const varData = d?.var ?? {}
  const varComparison = [
    { method: 'Parametric', value: (varData.parametric_95 ?? Number.NaN) * 100 },
    { method: 'Historical', value: (varData.historical_95 ?? Number.NaN) * 100 },
    { method: 'Monte Carlo', value: (varData.montecarlo_95 ?? Number.NaN) * 100 },
  ...
  return (
    <FeatureText><div>
      ...
      >
        <div className="overflow-x-auto">
          <table className="w-full border-collapse font-sans text-sm">
            ...
            <tbody>
              {ratioRows.map((row, i) => (
                <tr key={row.metric} className={i < ratioRows.length - 1 ? 'border-b border-border' : ''}>
      ...
      <ChartContainer
        ...
        subtitle={`${t('Daily returns', 'Rendimientos diarios')}: ${t('mean', 'media')}=${stat(distData.mean, 3, 100)}%, ${t('skew', 'asimetría')}=${stat(distData.skewness, 2)}, ${t('kurtosis', 'curtosis')}=${stat(distData.kurtosis, 2)}`}
      >
