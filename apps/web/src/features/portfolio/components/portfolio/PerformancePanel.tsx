...
import { usePerformance } from '@/features/portfolio/hooks/usePortfolioAPI'
...
function heatmapColor(value: number): string {
  if (value > 5) return 'var(--gain)'
  if (value > 2) return 'color-mix(in srgb, var(--gain) 50%, transparent)'
  if (value > 0) return 'color-mix(in srgb, var(--gain) 25%, transparent)'
  if (value > -2) return 'color-mix(in srgb, var(--loss) 25%, transparent)'
  if (value > -5) return 'color-mix(in srgb, var(--loss) 50%, transparent)'
  return 'var(--loss)'
}
        ...
        <div className="h-[420px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={cumulativeReturns}>
              ...
              <XAxis
                dataKey="date"
                tick={{ fill: 'var(--chart-tick)', fontSize: 11, fontFamily: 'var(--font-inter), sans-serif' }}
                tickLine={false}
                    ...
                    return (
                      <FeatureText key={mi}><td
                        ...
                        style={{
                          ...
                          color: pct == null ? 'var(--muted-dim)' : pct > 5 || pct <= -5 ? 'var(--paper)' : 'var(--ink)',
                        }}
                      >
