...
function Accordion({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <FeatureText><div className="border border-border rounded-lg overflow-hidden">
      <button
...
export function IntroSection() {
  return (
    <FeatureText><div className="mb-6 space-y-3">
      {/* Hero Context Box */}
      ...
      <Accordion title="Portfolio Composition">
        <div className="overflow-x-auto mt-3">
          <table className="w-full border-collapse font-sans text-sm">
            ...
            <tbody>
              {PORTFOLIO.map((row, i) => (
                <tr key={row.ticker} className={i < PORTFOLIO.length - 1 ? 'border-b border-border' : ''}>
