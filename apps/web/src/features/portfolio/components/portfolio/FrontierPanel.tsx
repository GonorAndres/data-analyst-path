...
import { useFrontier } from '@/features/portfolio/hooks/usePortfolioAPI'
...
function sharpeColorScale(sharpe: number): string {
  if (sharpe >= 1.5) return 'var(--gain)'
  if (sharpe >= 1.0) return 'var(--fixed-income)'
  if (sharpe >= 0.5) return 'var(--benchmark)'
  if (sharpe >= 0) return 'var(--muted)'
  return 'var(--loss)'
}
...
function FrontierTooltip({ active, payload }: FrontierTooltipProps) {
  ...
  return (
    <FeatureText><div className="p-3 rounded-lg border" style={{ background: 'var(--tooltip-bg)', borderColor: 'var(--tooltip-border)', fontFamily: 'var(--font-inter), sans-serif', fontSize: '13px', color: 'var(--ink)' }}>
      <p className="font-semibold">{p.label || 'Portfolio'}</p>
      <p className="font-mono tabular-nums">Return: {Number.isFinite(p.return) ? `${(p.return * 100).toFixed(2)}%` : '—'}</p>
      <p className="font-mono tabular-nums">Risk: {Number.isFinite(p.risk) ? `${(p.risk * 100).toFixed(2)}%` : '—'}</p>
      <p className="font-mono tabular-nums">Sharpe: {Number.isFinite(p.sharpe) ? p.sharpe.toFixed(3) : '—'}</p>
    </div></FeatureText>
  )
}
      ...
      >
        ...
        <div className="flex items-center gap-4 mt-4">
          ...
          <div className="flex items-center gap-1">
            {[
              { label: '< 0', color: 'var(--loss)' },
              { label: '0-0.5', color: 'var(--muted)' },
              { label: '0.5-1', color: 'var(--benchmark)' },
              { label: '1-1.5', color: 'var(--fixed-income)' },
              { label: '> 1.5', color: 'var(--gain)' },
            ].map(({ label, color }) => (
              ...
              <tbody>
                {[
                  ...
                  <tr key={row.label} className={i < arr.length - 1 ? 'border-b border-border' : ''}>
                    ...
                    <td className="py-3 px-4 text-right font-mono tabular-nums text-ink">{Number.isFinite(row.ret) ? `${(row.ret * 100).toFixed(2)}%` : '—'}</td>
                    <td className="py-3 px-4 text-right font-mono tabular-nums text-ink">{Number.isFinite(row.vol) ? `${(row.vol * 100).toFixed(2)}%` : '—'}</td>
                    <td className="py-3 pl-4 text-right font-mono tabular-nums font-semibold text-accent">{Number.isFinite(row.sr) ? row.sr.toFixed(3) : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
