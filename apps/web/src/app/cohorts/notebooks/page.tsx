...
export default function NotebooksPage() {
  ...
  const [activeId, setActiveId] = useSection('01', NOTEBOOKS.map(notebook => notebook.id))
  const active = NOTEBOOKS.find((n) => n.id === activeId)!

  return (
    <>
      <PageHeader
        eyebrow={tx("Proceso técnico")}
        title={tx("El código detrás de cada cifra")}
        lede={tx("Los cuatro notebooks completos, tal como se ejecutaron, con su código y sus salidas. Nada de lo que aparece en los tableros sale de un paso que no esté aquí.")}
      />
      ...
      <figure className="border border-border bg-[var(--chart-bg)] rounded-sm">
        <figcaption className="flex flex-wrap items-start justify-between gap-4 px-5 pt-4 pb-3">
          <div>
            ...
            <p className="font-sans text-sm text-muted mt-0.5 max-w-2xl">{tx(active.description)}</p>
          </div>
          <a
            href={active.src}
            target="_blank"
      ...
      <p className="font-sans text-xs text-muted">
        {tx("Los notebooks se exportan con") + " "}<code>jupyter nbconvert --to html</code> {" " + tx("y se publican tal cual. Traen su propio CSS, por lo que se muestran sobre fondo blanco en ambos temas: es el documento original, no una versión reestilizada para el tablero.")}</p>
    </>
  )
}
