...
import { formatCohort, formatExact } from '@/lib/format'
  ...
  if (rows.length === 0) {
    return (
      <p className="font-sans text-sm text-muted p-6">
        Ningún cohorte cumple los filtros actuales. Baja el tamaño mínimo de cohorte.
      </p>
    )
  }
    ...
    <div className="h-full overflow-auto px-3 pb-1">
      <table className="border-separate" style={{ borderSpacing: 2 }}>
        <caption className="sr-only">
          Porcentaje de clientes de cada cohorte que vuelve a comprar en los meses siguientes
        </caption>
        <thead>
          <tr>
                    ...
                    {isHover && v !== null && (
                      <div
                        ...
                        className="absolute z-10 left-1/2 -translate-x-1/2 bottom-full mb-1 whitespace-nowrap
                                   border border-border dark:border-[#2a2a2a] rounded-sm px-2 py-1 shadow-sm"
                        style={{ background: 'var(--chart-bg)' }}
                      >
                        <span className="font-sans text-xs text-[var(--chart-label)]">
                          {formatCohort(row.cohort)} · mes {m} ·{' '}
      ...
      <div className="flex items-center gap-2 mt-3 pl-1">
        <span className="font-sans text-[11px] text-muted">menor retención</span>
        {RAMP.map((c) => (
          <span
            key={c}
            aria-hidden
        ...
        <span className="font-sans text-[11px] text-muted">mayor</span>
      </div>
    </div>
  )
}
