...
export function ChartCard({
  ...
  return (
    <figure className="border border-border dark:border-[#2a2a2a] bg-[var(--chart-bg)] rounded-sm">
                ...
                <tr>
                  {tableColumns.map((c) => (
                    <th
                      key={c.key}
                      scope="col"
                      className={`py-2 px-2 border-b border-border dark:border-[#2a2a2a] font-medium text-muted ${
      ...
      {insight && (
        <div className="px-5 py-3 border-t border-border dark:border-[#2a2a2a] font-sans text-sm leading-relaxed text-muted">
          {insight}
        </div>
      )}
    </figure>
