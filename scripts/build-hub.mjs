 ...
 * site is publicly reachable at two addresses serving byte-identical content.
 *
 * This is a static `_redirects` file rather than a Function. A first attempt
 * used `functions/_middleware.ts`, which works -- it passed locally every time --
 * but it invokes the Functions runtime on every single request just to compare a
 * hostname, and on CI's 2-core runner that reliably killed the dev server
 * partway through the deploy gate. `_redirects` is evaluated at the edge before
 * any Function runs, so it costs nothing per request and cannot destabilise
 * anything.
 *
 * A full URL on the left is what makes this hostname-specific. A bare `/*` would
 * match on every hostname and redirect the site to itself. Preview deployments
 * live on `dev.` and `<hash>.` subdomains, which this exact host does not match.
 */
const redirects = [
  '# Generated by scripts/build-hub.mjs -- edit there, not here.',
  'https://data-analyst-hub.pages.dev/* https://data-analyst.gonor.me/:splat 301',
  '',
].join('\n')
fs.writeFileSync(path.join(DIST, '_redirects'), redirects)
log('\n_redirects: pages.dev apex -> data-analyst.gonor.me (301, edge-level)')
